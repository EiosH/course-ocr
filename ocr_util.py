...
# vLLM OpenAI 兼容服务地址，可通过环境变量覆盖
VLLM_BASE_URL = os.environ.get("VLLM_BASE_URL", "http://localhost:8000")
VLLM_API_KEY = os.environ.get("VLLM_API_KEY", "EMPTY")  # vLLM 默认不校验，随便填
...
def call_vllm_model(image_bytes, model_name, prompt_text, timeout=300):
    ...
    # token count makes vLLM prefill noticeably faster.
    # Full English OCR needs enough pixels for small slide and desktop text.
    max_dim = 1280
    ...
    try:
        ...
        raise VLLMConnectionError(error_msg) from e
    ...
    # HTTP 状态码非 2xx 时报错（例如模型不支持图片、参数错误等）
    if not response.ok:
        error_msg = (
        ...
        print(f"ERROR: {error_msg}")
        raise VLLMConnectionError(error_msg)
...
def process_single_image(image_bytes, model_name, prompt_text):
    ...
    content, prompt_tokens, completion_tokens = call_vllm_model(image_bytes, model_name, prompt_text)
...
def process_images_batch(image_tuples, model_name, prompt_text, max_workers=6):
    ...
    任意一张图片调用失败（含连接失败）都会中断整个批次并抛出 VLLMConnectionError。
    """
    results = []
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for future in concurrent.futures.as_completed(future_to_image):
            ...
            # 不吞异常：直接让 VLLMConnectionError / 其他异常向上抛出
...
def append_results_to_file(output_file_path, results, average_ocr_time=0):
    ...
    with open(output_file_path, "a", encoding="utf-8") as f:
        for result in results:
            ...
            f.write(f"\n{'='*60}\n")
            f.write(f"时间戳: {time_str} ({result['timestamp']:.2f}s), OCR耗时: {result['time']:.2f}s, 平均耗时: {average_ocr_time:.2f}s, InputTokens: {result.get('input_tokens', 0)}, OutputTokens: {result.get('output_tokens', 0)}\n")
            f.write(f"{'='*60}\n")
